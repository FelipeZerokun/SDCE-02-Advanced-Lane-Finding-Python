...
def main() -> None:
    ...
    left_fit_x, right_fit_x = evaluate_lane_fit(
        lane_fit,
        plot_y,
    )
    ...
    visualization[warped_binary != 0] = (64, 64, 64)
    ...
    left_curve = np.asarray(
        np.rint(
            np.column_stack((left_fit_x, plot_y)),
        ),
        dtype=np.int32,
    ).reshape((-1, 1, 2))
    ...
    right_curve = np.asarray(
        np.rint(
            np.column_stack((right_fit_x, plot_y)),
        ),
        dtype=np.int32,
    ).reshape((-1, 1, 2))
    ...
    _write_image(
        OUTPUT_DIRECTORY / "detector_result.jpg",
        visualization,
    )
    ...
    print(
        "Left coefficients:",
        lane_fit.left_coefficients,
    )
    print(
        "Right coefficients:",
        lane_fit.right_coefficients,
    )
    print(f"Saved detector diagnostic to: {OUTPUT_DIRECTORY}")
